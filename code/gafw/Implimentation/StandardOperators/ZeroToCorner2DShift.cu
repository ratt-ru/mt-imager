/* ZeroToCorner2DShift.cu:  CUDA implementation of the ZeroToCorner2DShift operator 
 *      
...
{
    namespace ZeroToCorner2DShift_kernels
    {
        ...
        template <class T>
        __global__ void ZeroToCorner2D(T * input, T * output, int3 dim);
...
using namespace GAFW::GPU::StandardOperators::ZeroToCorner2DShift_kernels;
using namespace GAFW::GeneralImplimentation;

void ZeroToCorner2DShift::submitToGPU(GAFW::GPU::GPUSubmissionData &data)
{
    ...
    dim3 threadsPerBlock;
     ...
     {
         case complex_float:
             ZeroToCorner2D<<<blocks,threadsPerBlock,0,data.stream>>>((cuComplex*)data.inputs[0].pointer, (cuComplex*)data.outputs->pointer, dim);
             break;
         case complex_double:
             ZeroToCorner2D<<<blocks,threadsPerBlock,0,data.stream>>>((cuDoubleComplex*)data.inputs[0].pointer, (cuDoubleComplex*)data.outputs->pointer, dim);
             break;
         default:
             break;
